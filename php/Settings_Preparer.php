<?php
	...
	class Settings_Preparer {
		...
		/**
		 * Registers ytvi settings with wordpress
		 */
		function settings_init() {
			...
			if (false == $options || empty($options)) {
				add_option('ytvi_options', apply_filters('ytvi_options', $this->default_options()));
			} else {
                            // Individual Option checking
                            $this->options_checking($options);
                        }
			
			register_setting('ytvi_options', 'ytvi_options');
		
			add_settings_section('ytvi_options_section',		            # id
                            'WP Youtube Video Importer Settings',		    # title
		...
		/*
		 * Callback output of form fields for ytvi options
		 */
		...
		function custom_video_width_callback() {
			...
			$html = "<p><input type='number' name='ytvi_options[video_width]' min='0' max='5000'"
		...
		function custom_video_height_callback() {
			...
			$html = "<input type='number' name='ytvi_options[video_height]' min='0' max='5000'"
